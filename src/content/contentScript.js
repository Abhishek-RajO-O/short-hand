chrome.storage.local.get(['allowedSites'], (result) => {
  const allowedSites = result.allowedSites || ['chatgpt.com'];
  const currentHost = window.location.hostname.toLowerCase();
  
  const isAllowed = allowedSites.some(site => currentHost.includes(site));
  
  if (!isAllowed) {
    return; // Silently exit if the current domain is not allowed
  }
